...
def _price_alerts_for_email(email: str, sim: Optional[dict] = None) -> tuple[dict, list[dict]]:
    """Load alerts with dedicated persistence, while keeping legacy sim_state fallback."""
    if sim is None:
        sim = _sim_state(email)

    # Primary store: dedicated telegram_price_alerts table.
    # This prevents a portfolio/sim_state rewrite from accidentally deleting alerts.
    if sb is not None:
        try:
            res = (
                sb.table("telegram_price_alerts")
                .select("*")
                .eq("actor", email)
            ...
            rows = res.data or []
            if rows:
                return sim, [dict(x) for x in rows if isinstance(x, dict)]
        except Exception as exc:
            # Table may not exist yet; use legacy sim_state below.
            print(f"[alerts] dedicated table unavailable: {exc}")
...
def _insert_price_alert(email: str, alert: dict, sim: dict, alerts: list[dict]) -> None:
    """Persist an alert independently from sim_state; legacy fallback if table unavailable."""
    if sb is not None:
        try:
            sb.table("telegram_price_alerts").insert({
                "id": str(alert["id"]),
                "actor": email,
                ...
                "last_price": float(alert["last_price"]) if alert.get("last_price") is not None else None,
            ...
            print(f"[alerts] dedicated insert failed, using sim_state: {exc}")
...
def _update_price_alert(alert: dict, email: str, sim: Optional[dict] = None) -> None:
    """Persist alert state changes to the dedicated table; fallback to sim_state."""
    if sb is not None:
        try:
            ...
            if aid:
                ...
                sb.table("telegram_price_alerts").update(payload).eq("id", aid).eq("actor", email).execute()
                return
        except Exception as exc:
            print(f"[alerts] dedicated update failed, using sim_state: {exc}")
...
def cmd_price_alert(chat_id: int, arg: str) -> str:
    ...
    active = [a for a in mine if a.get("active", True)]
    if len(active) >= 20:
    ...
    _insert_price_alert(email, alert, sim, alerts)
    ...
    try:
        ...
        for a in alerts:
            if str(a.get("id", "")).upper() == alert_id and int(a.get("chat_id") or 0) == int(chat_id):
                ...
                _update_price_alert(a, email, sim)
                found = True
                break
        if not found:
...
def check_price_alerts() -> None:
    """ตรวจ Price Alerts จาก dedicated table เพื่อไม่ให้หายเมื่อ sim_state ถูกเขียนทับ."""
    if sb is None:
        return
    try:
        res = (
        ...
        print(f"[alert] dedicated table unavailable: {exc}")
        # Legacy fallback for installations that have not created the table yet.
        try:
            ...
            for row in (res.data or []):
                ...
                if not email or not isinstance(sim, dict):
                    continue
                for a in (sim.get("price_alerts") or []):
                    if isinstance(a, dict) and a.get("active", True):
                        aa = dict(a)
                        aa["actor"] = email
                        rows.append(aa)
        except Exception as exc2:
            print(f"[alert] legacy load error: {exc2}")
            return
    ...
    for alert in rows:
        if not isinstance(alert, dict) or not alert.get("active", True):
            continue
        ...
        # Persist only the alert row, never the whole sim_state.
        _update_price_alert(alert, email, None)
