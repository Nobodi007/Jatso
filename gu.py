...
# =========================================================================
# PORTFOLIO HEATMAP — exposure / P&L visual map
# =========================================================================
def render_portfolio_heatmap(cfg: dict, data, market_df: pd.DataFrame):
    st.markdown("<div class='ph-title'>📊 Portfolio Heatmap</div>", unsafe_allow_html=True)
    st.caption("ภาพรวมสัดส่วนพอร์ตและ Unrealized P&L จาก Holdings ปัจจุบัน · Read-only")
    ...
    try:
        ...
        if market_df is not None and not market_df.empty:
            for _, row in market_df.iterrows():
                sym = str(row.get("symbol", "")).upper()
                try:
                    price_map[sym] = float(row.get("price_thb") or row.get("price", 0) or 0)
                except Exception:
                    pass
        snap = portfolio_snapshot(cfg.get("sim", {}), price_map)
    except Exception:
        snap = {"rows": [], "cash": 0.0, "total_value": 0.0, "unrealized_pnl": 0.0,
                "realized_pnl": 0.0, "fees": 0.0}

    rows = [r for r in (snap.get("rows") or []) if float(r.get("qty", 0) or 0) > 0]
    total = float(snap.get("total_value", 0) or 0)
    cash = float(snap.get("cash", 0) or 0)
    assets_value = sum(float(r.get("market_value", 0) or 0) for r in rows)
    total_pnl = float(snap.get("unrealized_pnl", 0) or 0) + float(snap.get("realized_pnl", 0) or 0)

    # CSS scoped to this page only.
    st.markdown("""
    <style>
    .ph-wrap{display:flex;flex-direction:column;gap:14px;margin:6px 0 18px}
    .ph-summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}
    .ph-card{border:1px solid rgba(148,163,184,.18);border-radius:16px;padding:16px 17px;background:linear-gradient(145deg,rgba(15,23,42,.82),rgba(30,41,59,.48));box-shadow:0 10px 28px rgba(0,0,0,.12)}
    .ph-label{font-size:.76rem;color:#94a3b8;text-transform:uppercase;letter-spacing:.05em}
    .ph-value{font-size:1.32rem;font-weight:800;margin-top:5px;color:#f8fafc}
    .ph-sub{font-size:.76rem;color:#94a3b8;margin-top:3px}
    .ph-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(235px,1fr));gap:12px}
    .ph-asset{position:relative;overflow:hidden;border:1px solid rgba(148,163,184,.18);border-radius:18px;padding:16px;background:rgba(15,23,42,.58);min-height:164px}
    .ph-asset:hover{border-color:rgba(96,165,250,.45);transform:translateY(-1px);transition:.15s ease}
    .ph-head{display:flex;align-items:center;justify-content:space-between;gap:10px}
    .ph-name{font-weight:800;color:#f8fafc;font-size:1rem}.ph-meta{font-size:.73rem;color:#94a3b8;margin-top:2px}
    .ph-pct{font-weight:800;font-size:.95rem;color:#cbd5e1}.ph-bar{height:7px;border-radius:99px;background:rgba(148,163,184,.12);overflow:hidden;margin:13px 0 12px}
    .ph-fill{height:100%;border-radius:99px;background:linear-gradient(90deg,#38bdf8,#818cf8)}
    .ph-row{display:flex;justify-content:space-between;gap:10px;font-size:.82rem;margin-top:6px}.ph-muted{color:#94a3b8}.ph-pos{color:#4ade80}.ph-neg{color:#fb7185}
    .ph-risk{font-size:.7rem;padding:4px 8px;border-radius:999px;background:rgba(245,158,11,.12);color:#fbbf24;border:1px solid rgba(245,158,11,.22)}
    .ph-cash{border-color:rgba(56,189,248,.22)}
    @media(max-width:760px){.ph-summary{grid-template-columns:repeat(2,minmax(0,1fr))}.ph-grid{grid-template-columns:1fr}.ph-value{font-size:1.08rem}}
    </style>
    """, unsafe_allow_html=True)

    if total <= 0:
        st.info("ยังไม่มีมูลค่าพอร์ตเพียงพอสำหรับแสดง Heatmap")
        return

    st.markdown('<div class="ph-wrap">', unsafe_allow_html=True)
    st.markdown('<div class="ph-summary">'
                f'<div class="ph-card"><div class="ph-label">Portfolio Value</div><div class="ph-value">฿{total:,.2f}</div><div class="ph-sub">Current marked value</div></div>'
                f'<div class="ph-card"><div class="ph-label">Crypto Exposure</div><div class="ph-value">{(assets_value/total*100):.2f}%</div><div class="ph-sub">{len(rows)} asset(s)</div></div>'
                f'<div class="ph-card"><div class="ph-label">Cash</div><div class="ph-value">{(cash/total*100):.2f}%</div><div class="ph-sub">฿{cash:,.2f}</div></div>'
                f'<div class="ph-card"><div class="ph-label">Total P&L</div><div class="ph-value">{"+" if total_pnl>=0 else ""}฿{total_pnl:,.2f}</div><div class="ph-sub">Realized + Unrealized</div></div>'
                '</div>', unsafe_allow_html=True)

    # Sort by allocation, then render one card per asset.
    rows = sorted(rows, key=lambda r: float(r.get("allocation_pct", r.get("pct", 0)) or 0), reverse=True)
    cards = []
    for r in rows:
        sym = str(r.get("sym") or r.get("symbol") or "?").upper()
        alloc = float(r.get("allocation_pct", r.get("pct", 0)) or 0)
        ...
        logo_html = (f'<img src="{logo}" style="width:34px;height:34px;border-radius:50%;object-fit:cover" alt="{sym}">' if logo
                     else f'<div style="width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:rgba(148,163,184,.16);font-weight:800">{sym[:1]}</div>')
        pnl_cls = "ph-pos" if pnl >= 0 else "ph-neg"
        flag = '<span class="ph-risk">High concentration</span>' if alloc >= 70 else ''
        cards.append(
            f'<div class="ph-asset"><div class="ph-head"><div style="display:flex;align-items:center;gap:10px">{logo_html}<div><div class="ph-name">{sym}</div><div class="ph-meta">{COIN_NAMES.get(sym,sym)}</div></div></div><div class="ph-pct">{alloc:.2f}%</div></div>'
            f'<div class="ph-bar"><div class="ph-fill" style="width:{min(max(alloc,0),100):.2f}%"></div></div>'
            f'<div class="ph-row"><span class="ph-muted">Market Value</span><b>฿{mv:,.2f}</b></div>'
            f'<div class="ph-row"><span class="ph-muted">Unrealized P&L</span><b class="{pnl_cls}">{"+" if pnl>=0 else ""}฿{pnl:,.2f} ({pnl_pct:+.2f}%)</b></div>'
            f'<div style="margin-top:9px">{flag}</div></div>'
        )
    if cash > 0:
        alloc = cash / total * 100
        cards.append(
            f'<div class="ph-asset ph-cash"><div class="ph-head"><div><div class="ph-name">THB Cash</div><div class="ph-meta">Available cash</div></div><div class="ph-pct">{alloc:.2f}%</div></div>'
            f'<div class="ph-bar"><div class="ph-fill" style="width:{min(max(alloc,0),100):.2f}%"></div></div>'
            f'<div class="ph-row"><span class="ph-muted">Cash Value</span><b>฿{cash:,.2f}</b></div></div>'
        )
    st.markdown('<div class="ph-grid">' + ''.join(cards) + '</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

    if rows:
        top = rows[0]
        st.caption(f"Top exposure: {str(top.get('sym') or top.get('symbol') or '—').upper()} · {float(top.get('allocation_pct', top.get('pct', 0)) or 0):.2f}% of current portfolio")


def render_portfolio_goals(cfg: dict[str, Any], data: pd.DataFrame, market_df: pd.DataFrame) -> None:
    """Portfolio Goals & Milestones — read-only analytics plus persisted user targets."""
    st.markdown("## 🎯 Portfolio Goals")
    st.caption("ตั้งเป้าหมายของพอร์ตและติดตามความคืบหน้า โดยไม่สร้าง Order หรือเปลี่ยน Holdings จริง")

    sim = cfg.get("sim") if isinstance(cfg, dict) else None
    if not isinstance(sim, dict):
        sim = cfg.setdefault("sim", {}) if isinstance(cfg, dict) else {}
    ensure_portfolio_ledger(sim)
    ...
    try:
        if isinstance(market_df, pd.DataFrame) and not market_df.empty:
            for _, r in market_df.iterrows():
                ...
                px = float(r.get("price_thb", r.get("price", 0.0)) or 0.0)
                if sym and px > 0:
                    price_map[sym] = px
    except Exception:
        pass
    ...
    snap = portfolio_snapshot(sim, price_map)
    total_value = float(snap.get("total_value", 0.0) or 0.0)
    total_pnl = float(snap.get("total_pnl", 0.0) or 0.0)
    cash = float(snap.get("cash", 0.0) or 0.0)
    cash_pct = (cash / total_value * 100.0) if total_value > 0 else 0.0

    goals = sim.setdefault("portfolio_goals", {})
    if not isinstance(goals, dict):
        goals = {}
        sim["portfolio_goals"] = goals

    target_value = float(goals.get("target_value", 0.0) or 0.0)
    target_pnl = float(goals.get("target_pnl", 0.0) or 0.0)
    target_cash_pct = float(goals.get("target_cash_pct", 10.0) or 10.0)
    monthly_contribution = float(goals.get("monthly_contribution", 0.0) or 0.0)
    goal_note = str(goals.get("note", "") or "")

    def _save_goals():
        goals["target_value"] = float(st.session_state.get("goal_target_value", 0.0) or 0.0)
        goals["target_pnl"] = float(st.session_state.get("goal_target_pnl", 0.0) or 0.0)
        goals["target_cash_pct"] = float(st.session_state.get("goal_target_cash", 10.0) or 10.0)
        goals["monthly_contribution"] = float(st.session_state.get("goal_monthly_contribution", 0.0) or 0.0)
        goals["note"] = str(st.session_state.get("goal_note", "") or "")
        goals["updated_at"] = datetime.now(timezone.utc).isoformat()
        save_sim_state(sim)

    with st.form("portfolio_goals_form"):
        c1, c2 = st.columns(2)
        with c1:
            st.number_input("🎯 Target Portfolio Value (THB)", min_value=0.0, step=10000.0,
                            value=target_value, key="goal_target_value")
            st.number_input("📈 Target Total P&L (THB)", min_value=0.0, step=5000.0,
                            value=target_pnl, key="goal_target_pnl")
        with c2:
            st.number_input("💵 Target Cash %", min_value=0.0, max_value=100.0, step=1.0,
                            value=target_cash_pct, key="goal_target_cash")
            st.number_input("💰 Planned Monthly Contribution (THB)", min_value=0.0, step=5000.0,
                            value=monthly_contribution, key="goal_monthly_contribution")
        st.text_input("📝 Goal Note", value=goal_note, key="goal_note", placeholder="เช่น เป้าหมายสิ้นปี 2026")
        st.form_submit_button("💾 Save Goals", on_click=_save_goals)
    ...
    m1, m2, m3, m4 = st.columns(4)
    with m1:
        metric_card("Portfolio Value", f"฿{total_value:,.2f}")
    with m2:
        metric_card("Target", f"฿{target_value:,.2f}" if target_value > 0 else "—")
    with m3:
        metric_card("Total P&L", f"฿{total_pnl:,.2f}")
    with m4:
        metric_card("Cash", f"{cash_pct:.2f}%")

    if target_value > 0:
        progress = max(0.0, min(1.0, total_value / target_value))
        remaining = max(0.0, target_value - total_value)
        st.progress(progress, text=f"Portfolio Goal · {progress*100:.1f}% complete")
        st.caption(f"เหลืออีก ฿{remaining:,.2f} เพื่อถึงเป้าหมายมูลค่าพอร์ต")
        if monthly_contribution > 0 and remaining > 0:
            months = remaining / monthly_contribution
            st.info(f"หากเติมเงินตามแผน ฿{monthly_contribution:,.2f}/เดือน จะใช้เวลาประมาณ {months:.1f} เดือน โดยยังไม่รวมผลตอบแทนจากตลาด")

    g1, g2 = st.columns(2)
    with g1:
        st.markdown("### 📈 P&L Goal")
        if target_pnl > 0:
            p = max(0.0, min(1.0, total_pnl / target_pnl))
            st.progress(p, text=f"{p*100:.1f}% of P&L target")
            st.caption(f"ปัจจุบัน ฿{total_pnl:,.2f} / เป้าหมาย ฿{target_pnl:,.2f}")
        else:
            st.caption("ยังไม่ได้ตั้งเป้าหมาย Total P&L")
    with g2:
        st.markdown("### 💵 Cash Target")
        diff = cash_pct - target_cash_pct
        st.metric("Current Cash", f"{cash_pct:.2f}%", delta=f"{diff:+.2f} pp vs target")
        st.caption(f"Target Cash: {target_cash_pct:.2f}%")

    st.markdown("### 🏁 Milestones")
    milestones = [0.25, 0.50, 0.75, 1.00]
    if target_value > 0:
        cols = st.columns(4)
        for col, ratio in zip(cols, milestones):
            milestone = target_value * ratio
            reached = total_value >= milestone
            with col:
                st.markdown(f"**{'✅' if reached else '○'} {ratio*100:.0f}%**")
                st.caption(f"฿{milestone:,.0f}")
    else:
        st.caption("ตั้ง Target Portfolio Value เพื่อเปิด Milestones")
...
NAV_HEATMAP = NAV_LABELS[18]
NAV_GOALS = NAV_LABELS[19]
NAV_ASSET_DETAIL = NAV_LABELS[20]
...
def render_asset_detail_center(cfg: dict[str, Any], data: pd.DataFrame, market_df: pd.DataFrame) -> None:
    st.markdown("## 🪙 Asset Detail")
    st.caption("เจาะรายละเอียดสินทรัพย์รายเหรียญจาก Portfolio / Wallet เดิม โดยไม่สร้างหรือแก้ธุรกรรม")
    ...
    if isinstance(market_df, pd.DataFrame) and not market_df.empty:
        for _, r in market_df.iterrows():
            ...
            if sym and px > 0:
                price_map[sym] = px
    try:
        asset_from_cfg = str(cfg.get("asset", "BTC")).upper()
        if isinstance(data, pd.DataFrame) and not data.empty and "Global_USD" in data.columns:
            price_map[asset_from_cfg] = float(data["Global_USD"].iloc[-1]) * usdthb
    except Exception:
        pass

    snap = portfolio_snapshot(sim, price_map)
    held_assets = [str(r.get("asset", "")).upper() for r in snap.get("rows", []) if str(r.get("asset", "")).upper()]
    watch_assets = [str(x).upper() for x in sim.get("watchlist", []) if str(x).upper() in SUPPORTED_ASSETS]
    choices = list(dict.fromkeys(held_assets + watch_assets + [str(cfg.get("asset", "BTC")).upper()]))
    choices = [x for x in choices if x in SUPPORTED_ASSETS] or SUPPORTED_ASSETS[:]

    default_asset = str(st.session_state.get("asset_detail_symbol", choices[0])).upper()
    if default_asset not in choices:
        default_asset = choices[0]
    selected = st.selectbox(
        "เลือกสินทรัพย์", choices, index=choices.index(default_asset),
        key="asset_detail_selector",
        format_func=lambda x: f"{x} · {COIN_NAMES.get(x, x)}",
    )
    st.session_state["asset_detail_symbol"] = selected

    row = next((r for r in snap.get("rows", []) if str(r.get("asset", "")).upper() == selected), None)
    px = float(price_map.get(selected, 0.0) or 0.0)
    qty = float(row.get("qty", 0.0) or 0.0) if row else 0.0
    avg = float(row.get("avg_cost", 0.0) or 0.0) if row else 0.0
    cost = float(row.get("cost_basis", 0.0) or 0.0) if row else 0.0
    mv = float(row.get("market_value", 0.0) or 0.0) if row else 0.0
    upnl = float(row.get("unrealized_pnl", 0.0) or 0.0) if row else 0.0
    alloc = float(row.get("allocation_pct", 0.0) or 0.0) if row else 0.0
    pnl_pct = (upnl / cost * 100.0) if cost > 0 else 0.0

    st.markdown(
        f'''<div style="display:flex;align-items:center;gap:14px;padding:18px 20px;margin:8px 0 14px;border:1px solid #2b3139;border-radius:16px;background:linear-gradient(135deg,#14181d,#0f1216);">
          {coin_icon_html(selected, 52)}
          <div style="flex:1;min-width:0;"><div style="font-size:20px;font-weight:800;color:#EAECEF;">{_html.escape(selected)}</div><div style="font-size:12px;color:#848e9c;">{_html.escape(COIN_NAMES.get(selected, selected))}</div></div>
          <div style="text-align:right;"><div style="font-size:11px;color:#848e9c;">ราคาปัจจุบัน</div><div style="font-size:22px;font-weight:800;color:#EAECEF;">฿{px:,.2f}</div></div>
        </div>''', unsafe_allow_html=True,
    )

    m1, m2, m3, m4, m5 = st.columns(5, gap="small")
    _portfolio_metric_card(m1, "ถืออยู่", fmt_coin(qty, selected))
    _portfolio_metric_card(m2, "Average Cost", f"฿{avg:,.2f}")
    _portfolio_metric_card(m3, "Market Value", f"฿{mv:,.2f}")
    _portfolio_metric_card(m4, "Unrealized P&L", fmt_baht_full(upnl, True), _portfolio_pnl_class(upnl))
    _portfolio_metric_card(m5, "Allocation", f"{alloc:.2f}%")

    c1, c2 = st.columns([1.35, 1], gap="large")
    with c1:
        st.markdown("### 📈 Price History")
        period = st.selectbox("ช่วงข้อมูล", ["30D", "90D", "180D", "1Y"], index=1, key="asset_detail_period")
        days = {"30D": 30, "90D": 90, "180D": 180, "1Y": 365}[period]
        end_date = pd.Timestamp.utcnow().tz_localize(None).normalize()
        start_date = end_date - pd.Timedelta(days=days)
        hist, hist_err = fetch_price_data(selected, start_date, end_date, use_fx_proxy=False)
        if hist_err or hist.empty:
            st.info(f"ยังไม่สามารถโหลดกราฟ {selected} ได้: {hist_err or 'ไม่มีข้อมูล'}")
        else:
            h = hist.copy()
            h["Price_THB"] = h["Global_USD"] * h["USDTHB"]
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=h.index, y=h["Price_THB"], mode="lines", name=f"{selected}/THB", line=dict(width=2)))
            fig.update_layout(height=310, margin=dict(l=8, r=8, t=10, b=8), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(color="#B7BDC8"), xaxis=dict(showgrid=False), yaxis=dict(showgrid=True, gridcolor="#252a31"), hovermode="x unified", showlegend=False)
    ...
    with c2:
        ...
        st.markdown(
            ...
            <div class="portfolio-summary-strip"><span>ต้นทุนคงเหลือ</span><b>฿{cost:,.2f}</b></div>
            <div class="portfolio-summary-strip"><span>มูลค่าปัจจุบัน</span><b>฿{mv:,.2f}</b></div>
            <div class="portfolio-summary-strip"><span>กำไร/ขาดทุน</span><b class="{_portfolio_pnl_class(upnl)}">฿{upnl:+,.2f} · {pnl_pct:+.2f}%</b></div>
            <div class="portfolio-summary-strip"><span>สัดส่วนพอร์ต</span><b>{alloc:.2f}%</b></div>
            </div>''', unsafe_allow_html=True,
        )
        if row:
            st.progress(min(max(alloc / 100.0, 0.0), 1.0), text=f"Portfolio Allocation · {alloc:.2f}%")
        else:
            st.info("ยังไม่มี Holdings ของเหรียญนี้ — กำลังดูในโหมด Watchlist")
        if st.button(f"↗️ เปิด {selected}/THB ใน Exchange", use_container_width=True, key="asset_detail_exchange"):
            _go_to_exchange(selected)
            st.rerun()

    st.markdown("### 🧾 Transaction History — " + selected)
    txs = [tx for tx in snap.get("transactions", []) if str(tx.get("asset", "")).upper() == selected]
    if txs:
        txdf = pd.DataFrame(txs)
        txdf["timestamp"] = pd.to_datetime(txdf["timestamp"], errors="coerce")
        txdf = txdf.sort_values("timestamp", ascending=False)
        cols = [c for c in ["timestamp", "type", "qty", "price_thb", "gross_thb", "fee_thb", "cash_delta_thb", "realized_pnl_thb", "note"] if c in txdf.columns]
        st.dataframe(txdf[cols], use_container_width=True, hide_index=True)
    else:
        st.info("ยังไม่มีธุรกรรมของเหรียญนี้")

    st.caption("ข้อมูลราคาเป็นราคาตลาดล่าสุดที่แอปโหลดได้ · P&L และต้นทุนอ่านจาก Portfolio Ledger เดิม · หน้านี้เป็นข้อมูลประกอบ ไม่สร้าง Order")
...
def _main_body() -> None:
    ...
    with st.container(key="desktop_route"):
        ...
        elif nav == NAV_HEATMAP:
            render_portfolio_heatmap(cfg, data, market_df)
        elif nav == NAV_GOALS:
            render_portfolio_goals(cfg, data, market_df)
        elif nav == NAV_ASSET_DETAIL:
            render_asset_detail_center(cfg, data, market_df)
        elif nav == NAV_SIMPLE:
